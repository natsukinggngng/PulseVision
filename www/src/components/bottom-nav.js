...
class BottomNavigation {
  ...
  init() {
    const iconMap = {
      ...
      'rutas': '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>',
      'historial': '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>',
        ...
        ${this.items.map(item => `
          ...
          >
            <span class="nav-icon">
              ...
              ${item.id === 'chat' ? '<span class="nav-badge" hidden>0</span>' : ''}
            </span>
  ...
  /**
   * Muestra u oculta el badge numérico de un ítem (p. ej. chat no leído)
   * @param {string} navId
   * @param {number} count
   */
  setBadge(navId, count) {
    const button = this.container.querySelector(`.bottom-nav-item[data-nav="${navId}"]`);
    if (!button) return;
